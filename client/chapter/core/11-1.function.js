...
// 2. 전역의 오염을 막는다.
// 3. 하나의 기능만을 수행해야 한다.
// 4. 재사용성이 좋아야 한다. (매개변수 활용, 유연한 함수를 만들어야 한다.)
...
//getComputedStyle 정의된 스타일 값을 가져오는 함수
//getComputedStyle(first)['backgroundColor']first에 정의된 백그라운드스타일 값을 가져온다

//1. function name
//2. argumenmt 함수 실행부 미리 작성
//3. parameter
//4. return value
//5. validation
...
const h1FonstSize = getStyle('.first', 'fontSize');

function css(node, prop, velue) {
  // if (!velue) {
  //   //geter
  //   return getStyle(node, prop);
  // } else {
  //   //seter
  //   setStyle(node, prop, velue);
  // }
  return !velue ? getStyle(node, prop) : setStyle(node, prop, velue);
}
