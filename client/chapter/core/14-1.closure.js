...
function first() {
  ...
  function second() {
    ...
    return x + y; //안쪽 대상이 바깥쪽을 사용해야 클로저 사용가능
  }

  return second; //함수 본문을 리턴
}

const result = first(); // 30 출력

result(); //함수 실행

//가비지컬렉터가 수집할 수 없게 만드는 환경 구성
/* 실행 설명 */
/*
1. fisrt 함수 안에 second 함수를 작성
2. first함수 영역에 return second 을 사용하여 second 함수 본분 전체를 리턴하고
3. first()를 실행하면 second 함수를 리턴한 값이 나온다.
4. second 함수 본문이 리턴되었으니까  first()(); 처럼 실행할 경우 second() 함수가 실행된다.
5. 가독성 좋게 만들기 위해 result 변수에 담아서 사용할 수 있다 
6. 이러한 함수 환경 구성을 클로저라고 한다. 
*/
...
// 클로저를 사용해 변수 할당의경우 매번 다른 환경 구성 가능
// const c1 = counter();
// const c2 = counter();
// const c3 = counter();
